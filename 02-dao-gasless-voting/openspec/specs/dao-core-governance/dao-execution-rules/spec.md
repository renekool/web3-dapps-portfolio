...
### Requirement: Sincronización Temporal Híbrida (UI Assurance)
Para mitigar la latencia de bloque o estados estáticos de la red (localnodes), la lógica de ejecución en la UI debe implementar un diferencial acumulativo.

- **WHEN** la dApp detecta un Timelock activo.
- **THEN** debe utilizar un contador de offset local para habilitar el gatillo de transacción exactamente al cumplirse el plazo, evitando llamadas prematuras que resultarían en REVERT de red y pérdida innecesaria de UX.
- **SYNC RULE**: Se debe persistir el `lastSyncTime` global para que el estado de tiempo sea resistente a recargas de página y el usuario no se vea atrapado en bucles de reseteo de temporizador.
