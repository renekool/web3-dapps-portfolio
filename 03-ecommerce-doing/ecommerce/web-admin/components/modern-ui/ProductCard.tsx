...
export const ProductCard = ({ product, onToggle, onEdit, canEdit = true }: ProductCardProps) => {
      ...
      <div className="flex items-center justify-between gap-2 mt-auto">
        ...
        <div className="flex items-center gap-2">
          <Switch
            ...
            disabled={!canEdit}
            className="data-[state=checked]:bg-[#A8432F] data-[state=unchecked]:bg-[#EEEBE7] disabled:opacity-40"
          />
          <button
            onMouseEnter={() => canEdit && setEHov(true)}
            onMouseLeave={() => setEHov(false)}
            onClick={() => canEdit && onEdit(product)}
            disabled={!canEdit}
            className="w-[30px] h-[30px] rounded-[8px] flex items-center justify-center transition-all duration-150"
            style={{
              cursor: canEdit ? 'pointer' : 'not-allowed',
              opacity: canEdit ? 1 : 0.4,
              border: `1.5px solid ${eHov ? '#C8B8B0' : T.border}`,
              background: eHov && canEdit ? T.terraLt : 'transparent',
            }}
          >
            <IcEdit color={eHov && canEdit ? T.terraIco : T.text3} />
          </button>
        </div>
      </div>
    </div>
